...
//FontAwesome
export default class dynamicMine extends Component {
	...
	article1(token)
	{
		...
		javaBase.requestToken({
			...
			body: {method: 'post', params:{pageNum: 1, pageSize: 5, status: 2, isMine: 1, token: token, }, },
		})
			.then((res) => {
				...
				if (res.code==1)
				{
					...
					if (dataList.length > 0)
					{
						...
						for (var i = 0; i < dataList.length; i++)
						{
							...
							temp["auditReason"] = dataList[i].auditReason;
							if (dataList[i].photoUrls != null) {
								temp["image"] = (dataList[i].photoUrls.split(','));
							}
							...
							temp["time"] =that.format(dataList[i].createTime);
							temp["isDeleted"] = dataList[i].isDeleted;
	...
	article2(token)
	{
		...
		javaBase.requestToken({
			path: "/weixin/circle/list",
			body: {method: 'post', params: {pageNum: 1, pageSize: 5, status: 1, isMine: 1, token: token, }, },
		})
			.then((res) => {
				console.log(res, '直至栀子花资质')
				if(res.code==1)
	...
	article3(token)
	{
		...
		javaBase.requestToken({
			path: "/weixin/circle/list",
			body: {method: 'post', params: {pageNum: 1, pageSize: 5, status: 3, isMine: 1, token: token, }, },
		})
		.then((res) => {
			console.log(res, '直至栀子花资质')
			if(res.code==1)
	...
	refreshList1 = async ()=>
	{
		...
		javaBase.request({
			...
			.then((responseData)=>
			{
				...
				{
					var dataList = responseData.data.data;
					if (dataList.length > 0)
					{
						var temp
						for (var i = 0; i < dataList.length; i++)
						{
							...
							temp["time"] =this.format(dataList[i].createTime);
							temp["isDeleted"] = dataList[i].isDeleted;
							temp["statusCode"]=dataList[i].statusCode;
							//根据帖子不同状态 放入对应的list 中
							list2.push(temp);
	...
	renderList2Row = (item, index) =>
	{
		return (<View key={index}>
			<TouchableOpacity onPress={this.detail.bind(this, item)} style={styles.listChild}>
				<Text style={styles.titleChild}>{item.title}</Text>
				<View style={styles.imgContent}>
					{
						item.image.map((img, i) =>{
							return (<View key={i} style={styles.imgChild}>
	...
	renderList3Row = (item, index) => {
		...
		return (<TouchableOpacity onPress={this.detail.bind(this, item)} style={styles.listChild} key={index}>
			<Text style={styles.titleChild}>{item.title}</Text>
			<View style={styles.imgContent}>
				{
					item.image.map((img, i) =>{
			...
			<View style={styles.violation}>
				<Text style={{fontSize: 13,}}>此文章涉嫌违禁，已被平台删除！如有疑问，请联系客服人员。</Text>
			</View>
		</TouchableOpacity>)
	}
