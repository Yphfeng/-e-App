...
//FontAwesome
class HotResult extends Component
{
	...
	async componentDidMount()
	{
		...
		{
			var searchValue = this.props.navigation.state.params.searchValue;
			this.setState({
	...
	_fetchData = async (page) =>
	{
		...
		params.pageSize = 5
		params.pageNum = 1
		params.labelName = this.state.searchValue;
		state.params = params
		javaBase.requestToken({
			path: '/Weixin/Circle/list',
			body: state,
	...
	_keyboardDidShow(e)
	{
		...
		if (this._textInput && this._textInput.isFocused())
		{
		...
		if (len > 0)
		{
			for (var i = 0; i<len; i++)
			{
				...
				else
				{
					if (item.wxUserName)
					{
						didItem.userName = item.wxUserName
					}
					else
					{
				...
				{
					if (item.wxHeadPortrait)
					{
						didItem.avatar = item.wxHeadPortrait
					}
					else
					{
						didItem.avatar = ""
		...
		return (
			<View style={{flex: 1, backgroundColor: "#f3f3f3", }}>
				...
				<View style={styles.header}>
					...
					><Icon name="ios-arrow-back" size={28} color={'#fff'} ></Icon></TouchableOpacity>
...
export default connect(mapStateToProps, mapDispatchToProps)(HotResult)
