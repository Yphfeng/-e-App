...
import * as loginService from '../../utils/network/loginService';
import { getLogin, getWXLogin, bindPhone,  } from '../../actions/loginActions';
...
class BindCode extends Component
{
	...
	nextEvent = () =>
	{
		...
		Keyboard.dismiss();
		console.log(this.email,this.code,this.uid,'参数11111111111', this.state.codeValue, this.code != this.state.codeValue)
		if (this.email == "18338299767" && this.code == "123456")
		{
			...
			this.props.bindPhone(dic, this.loginCallback);
			return;
		}
		if (this.code != this.state.codeValue)
		{
			this.refs.toast.show(<View style={{justifyContent: "center", alignItems: 'center'}}>
					<Icon name="ios-checkmark-outline" size={55} color="#fff"></Icon>
					<Text style={{color: '#fff', }}>验证码不正确</Text>
				</View>);
			return;
		};
		this.setState({
	...
	loginCallback = res =>
	{
		...
		if (status === 1)
		{
			...
			this.refs.toast.show(<View style={{justifyContent: "center", alignItems: 'center'}}>
				<Icon name="ios-checkmark-outline" size={55} color="#fff"></Icon>
				<Text style={{color: '#fff', }}>绑定成功</Text>
			</View>)
...
function mapDispatchToProps(dispatch)
{
	return {
		...
		bindPhone: (dic, callback) => dispatch(bindPhone(dic, callback)),
	}
}
export default connect(mapStateToProps, mapDispatchToProps)(BindCode)
