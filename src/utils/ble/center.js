...
export default class Center
{
	...
	async scan(s)
	{
		try
		{
			const result = BleManager.scan([], s, true);
			return result
		}
		catch (error)
		{
	...
	async stopScan()
	{
		try
		{
			const result = BleManager.stopScan();
		}
		catch (error)
		{
	...
	async connect(id)
	{
		...
		try
		{
			var result = await BleManager.connect(id);
			// var peripheralInfo = await BleManager.retrieveServices(id)
			var peripheralInfo;
			if (Platform.OS == "android")
			{
				for (var i = 0 ; i< 10; i++)
				{
					...
					console.log(peripheralInfo, "获取的retreservices")
			...
			console.log(peripheralInfo, "连接成功的返回")
	...
	async disconnect()
	{
		try
		{
			const result = await BleManager.disconnect(this.peripheralId);
		}
		catch (error)
		{
	...
	async startNotification(index = 0)
	{
		try
		{
			...
			const result = await BleManager.startNotification(this.peripheralId, this.nofityServiceUUID[index], this.nofityCharacteristicUUID[index]);
			return result
		}
		catch (error)
		{
	 ...
	 * Start the notification on the specified characteristic.
	 * */
	async startDFUNotification(index = 0)
	{

		try
		{
			const result = await BleManager.startNotification(this.peripheralId, "FE59", "8EC90003-F315-4F60-9FB8-838830DAEA50");
			return result
		}
		catch (error)
		{
	...
	async write(data, index)
	{
		...
		try
		{
			console.log(this.peripheralId, this.writeWithResponseServiceUUID[index], this.writeWithResponseCharacteristicUUID[index])
			const result = await BleManager.write(this.peripheralId, this.writeWithResponseServiceUUID[index], this.writeWithResponseCharacteristicUUID[index], data);
	...
	/**
	 * 写空中升级数据到蓝牙兼容ios和android
	 * 参数：(peripheralId, serviceUUID, characteristicUUID, data, maxByteSize)
	 * Write with response to the specified characteristic, you need to call retrieveServices method before.
	 * */
	async writeDFU(data, index)
	{
		if (Platform.OS == 'android')
		{
			index = 1
			writeWithResponseServiceUUID = '0000FE59-0000-1000-8000-00805F9B34FB'
		} else
		{
			index = 0
			writeWithResponseServiceUUID = 'FE59'
		}
		//writeWithResponseServiceUUID = "0000FE59-0000-1000-8000-00805F9B34FB"
		//writeWithResponseCharacteristicUUID = "8EC90003-F315-4F60-9FB8-838830DAEA50"
		try {
			console.log(this.peripheralId, this.writeWithResponseServiceUUID[index], this.writeWithResponseCharacteristicUUID[index])
			const result = await BleManager.write(this.peripheralId, writeWithResponseServiceUUID, '8EC90003-F315-4F60-9FB8-838830DAEA50', data);
			console.log(result,"写入数据的结果")
			return result
		} catch (error) {
			console.log(error, 'dfu写入指令的错误')
			return error
		}
		// data = this.addProtocol(data);   //在数据的头尾加入协议格式，如0A => FEFD010AFCFB，不同的蓝牙协议应作相应的更改
	}
